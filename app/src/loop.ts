  ...
  const results = await Promise.all(
    twitchInfo.map(async (info) => {
      try {
        ...
        console.error(`⚠️ Erreur réseau lors de la vérification de ${info.username} :`, error.message);
        return true; // On retourne true pour continuer la boucle sans déclencher un refresh de token
      }
    }),
  );
